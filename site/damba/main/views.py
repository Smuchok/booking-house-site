...
map_legend = {
    'name':"Карта",
    'legend':['Парковка', 'Рецепція', 'Дитяча площадка', 'Пляж', 'Вольєр з оленями', 'Адміністративна споруда', 'Адміністративна споруда'],
    }
...
def order_house(request, house_id):
    ...
    def today_date():
        ...
        return [year, month if month>9 else '0'+str(month), month+1 if month+1>9 else '0'+str(month), months[str(month)], months[str(month+1)]]
    ...
    context = {
        ...
        'dates': {
            ...
            'current_month_num': today[1],
            'next_month_num': today[2],
